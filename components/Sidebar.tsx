...
const Sidebar: React.FC = () => {
  ...
  const menuItems = [
    ...
    { id: 'activity-log', label: 'الرقابة', icon: '🛡️' }, // This will navigate to ActivitiesList
  ...
  return (
    ...
    >
      ...
      >
        <span className={`text-xl transition-transform duration-500 ${isSidebarCollapsed ? 'rotate-180' : 'rotate-0'}`} aria-hidden="true">
          ←
        </span>
      </button>
      ...
      <div className={`p-8 lg:p-10 flex flex-col h-full transition-all duration-500 ${isSidebarCollapsed ? 'items-center' : ''}`}>
        ...
        <div className={`flex items-center gap-6 mb-16 group cursor-pointer transition-all duration-500 ${isSidebarCollapsed ? 'justify-center' : ''}`} onClick={() => navigate('dashboard')}>
          <div className={`rounded-2xl flex items-center justify-center shadow-2xl border-4 border-white/20 group-hover:scale-110 transition-all ${isSidebarCollapsed ? 'w-14 h-14 text-2xl' : 'w-18 h-18 text-3xl'} ${theme === 'dark' ? 'bg-gradient-to-br from-[var(--color-accent-emerald)] to-[var(--color-accent-emerald)]/80' : 'bg-gradient-to-br from-[var(--color-accent-sky)] to-[var(--color-accent-info)]'}`} aria-hidden="true">🌿</div>
          {!isSidebarCollapsed && (
        ...
        <nav className="flex-1 space-y-2">
          {menuItems.map((item) => (
            ...
            >
              <span className={`${isSidebarCollapsed ? 'text-3xl' : 'text-2xl'}`} aria-hidden="true">{item.icon}</span>
        ...
        <div className={`mt-10 pt-8 border-t border-[var(--color-border-primary)] space-y-4`}>
          ...
          <button 
            ...
            className={`w-full flex items-center gap-6 rounded-2xl font-black text-[var(--color-accent-rose)] hover:bg-[var(--color-accent-rose)]/10 transition-all ${isSidebarCollapsed ? 'justify-center p-4' : 'px-6 py-4 text-sm'}`}
          >
